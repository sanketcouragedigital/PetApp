package com.couragedigital.peto.Services;
...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private void sendNotification(RemoteMessage remoteMessage) {
        if(Objects.equals(remoteMessage.getData().get("PET_NOTIFICATION_TYPE"), "OPEN_ACTIVITY_PET_DETAILS")) {
            intent = new Intent(this, PetListDetails.class);
            ...
            notificationBuilder = (NotificationCompat.Builder) new NotificationCompat.Builder(this)
                    .setSmallIcon(R.drawable.ic_launcher_short)
        ...
        else if(Objects.equals(remoteMessage.getData().get("PET_NOTIFICATION_TYPE"), "OPEN_ACTIVITY_PET_MATE_DETAILS")) {
            intent = new Intent(this, PetMateListDetails.class);
            intent.putExtra("PET_FIRST_IMAGE", remoteMessage.getData().get("PET_FIRST_IMAGE"));
            intent.putExtra("PET_SECOND_IMAGE", remoteMessage.getData().get("PET_SECOND_IMAGE"));
            intent.putExtra("PET_THIRD_IMAGE", remoteMessage.getData().get("PET_THIRD_IMAGE"));
            intent.putExtra("PET_MATE_BREED", remoteMessage.getData().get("PET_MATE_BREED"));
            intent.putExtra("PET_MATE_AGE_MONTH", remoteMessage.getData().get("PET_MATE_AGE_MONTH"));
            intent.putExtra("PET_MATE_AGE_YEAR", remoteMessage.getData().get("PET_MATE_AGE_YEAR"));
            intent.putExtra("PET_MATE_GENDER", remoteMessage.getData().get("PET_MATE_GENDER"));
            intent.putExtra("PET_MATE_DESCRIPTION", remoteMessage.getData().get("PET_MATE_DESCRIPTION"));
            intent.putExtra("POST_OWNER_MOBILENO", remoteMessage.getData().get("POST_OWNER_MOBILENO"));
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            PendingIntent pendingIntent = PendingIntent.getActivity(this, 0 /* Request code */, intent,
                    PendingIntent.FLAG_ONE_SHOT);
            ...
            notificationBuilder = (NotificationCompat.Builder) new NotificationCompat.Builder(this)
                    .setSmallIcon(R.drawable.ic_launcher_short)
                    .setLargeIcon(BitmapFactory.decodeResource(this.getResources(),R.drawable.ic_launcher))
                    .setContentTitle(remoteMessage.getData().get("PET_MATE_BREED"))
                    .setContentText(remoteMessage.getData().get("PET_MATE_DESCRIPTION"))
                    .setPriority(2)
                    .setStyle(new NotificationCompat.BigTextStyle().bigText(remoteMessage.getData().get("PET_MATE_DESCRIPTION")))
                    .setAutoCancel(true)
                    .setSound(defaultSoundUri)
                    .setLights(Color.GREEN, 1000, 1000)
                    .setContentIntent(pendingIntent);
